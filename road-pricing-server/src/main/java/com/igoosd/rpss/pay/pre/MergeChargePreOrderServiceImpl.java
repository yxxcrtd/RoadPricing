 ...
 */
@Service
public class MergeChargePreOrderServiceImpl extends AbsPreOrderService {


    @Autowired
    private ArrearsChargePreOrderServiceImpl arrearsChargePreOrderService;

    @Autowired
    private ParkingChargePreOrderServiceImpl parkingChargePreOrderService;
    ...
    protected List<TOrderItem> getOrderItemsForBiz( BusinessInfo businessInfo) {
        try {
            ...
            throw new RoadPricingException("预下单" + getOrderTypeEnum().getName() + "异常；明细如下：" + e.getMessage());
        }
    }
